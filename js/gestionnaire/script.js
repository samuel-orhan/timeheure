...
function defineTime() {
    $('#content').find('.timer').each(function () {
        var date = new Date();
        date.setTime(new Date().getTime() + dateDecalage);

        $(this).html(date.getHours() + ':' + (date.getMinutes() < 10 ? '0' : '') + date.getMinutes());
    });
}

function loadTimer() {
    window.open('/minitimeheure', "TIMER", "menubar=no, status=no, scrollbars=no, menubar=no, width=360, height=480")
}

function getTime(idprojet) {
    $.getJSON('/gestionprojets/get_temps_projet/' + idprojet, function (data) {
        var dialog = $.alert(data.temps_personnes);
        dialog.setHeader('Détails des temps passés :');
    });
}
function sateStateProject(idProjet, state) {
    $.getJSON('/gestionprojets/etatprojet/' + idProjet + '/' + state, function (data) {
        var datatable = $('#content').find("table.projet").dataTable();
...
$(document).ready(function () {
    ...
    if ($content.find('#time').length > 0) {
        dateDecalage = parseInt($('#time').val()) * 1000 - new Date().getTime();

        setInterval('defineTime();', 500);
    }
    ...
    $content.find("table.client, table.hebergeur").dataTable({
        "sPaginationType": "full_numbers"
    });

    $content.find("table.projet").dataTable({
        "sPaginationType": "full_numbers",
        "aoColumnDefs": [
        ...
        "bProcessing": true,
        "bServerSide": true,
        "sAjaxSource": "/gestionprojets/get_projets" + ( $('#projetlist').val() ? '/'+$('#projetlist').val() : '')
    });
    ...
    $content.find('div.calendar').wl_Calendar({
        ...
        events: $content.find('div.calendar').attr('rel')
    });
